            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <div class="tag_div" style="position: relative;">  
                        ...
                        <select id="select-tag" placeholder="Search Tag..." name="solution_tags[]" multiple>
                            @if(!empty($tags))
                                @foreach($tags as $tag)
                ...
                <div class="col-md-6">
                    ...
                    <select id="select-disease" name="solution_diseases[]" multiple placeholder="Search Disease...">
                        @if(!empty($selected_diseases) && $selected_diseases->count())
                            @foreach($selected_diseases as $disease)
                                <option value="{{ $disease->disease_id }}" selected>{{ $disease->disease_title }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
...
new TomSelect('#select-disease', {
    ...
    items: @json(($selected_diseases ?? collect())->pluck('disease_id')),
    onItemAdd: function() {
